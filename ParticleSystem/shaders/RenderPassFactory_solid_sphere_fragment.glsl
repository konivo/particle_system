...
subroutine void SetFragmentDepth(vec3 pos, vec4 projected);
...
void FragDepthDefault(vec3 intersection, vec4 projected)
{
	gl_FragDepth =  (projected.z + 1) * 0.5;
}

///////////////
subroutine(SetFragmentDepth)
void FragDepthExponential(vec3 intersection, vec4 projected)
{
	gl_FragDepth =  exp(((projected.z + 1) * 0.5) * EXP_SCALE_FACTOR - EXP_SCALE_FACTOR);
}
...
void main ()
{
	...
	// Setup the outputs
	u_SetFragmentDepth(intersection, projected_i);
}
